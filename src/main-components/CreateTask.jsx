...
import { isEmpty, get, map, merge, isNil, kebabCase } from 'lodash';
...
class CreateTask extends Component {
  ...
  updateFieldValue(event, field) {
    const oldTaskState = { ...{}, ...this.state.oldTaskState };
    oldTaskState[field] = event.target.value;
    this.setState({
  ...
  createtask() {
    if (this.istaskDataValid()) {
      ...
      const taskDBReference = isNil(this.state.oldTaskState)
        ? `${this.state.projectUrl}/tasks/${
            this.state.taskNameSlug
          }-${timestamp}`
      ...
      this.taskRef.set({
        ...
        highPriorityTask: this.state.highPriorityTask,
        taskNameSlug:
          this.state.taskNameSlug || `${this.state.taskNameSlug}-${timestamp}`,
      ...
      <div>
        <main role="main" className="container-fluid">
          {this.state.redirect || !this.state.user || !this.state.projectUrl ? (
            <Redirect
              to={`/projects/${
                this.state.projectUrl ? this.state.projectUrl : ''
              }`}
            />
          ) : (
            <div />
            ...
            <div className="form create-project create-task col-md-8">
              ...
              {this.state.createdBy ? (
                <small className="text-muted">
                  created by {`${this.state.createdBy}`} at{' '}
